...
var unlocked_stage = 2  # padrão (salvo será lido no _ready)
...
func _update_stage_buttons():
	fase1_button.disabled = unlocked_stage < 1
	fase2_button.disabled = false
	fase3_button.disabled = unlocked_stage < 3

func _on_fase1_pressed():
	_play_click()
	get_tree().change_scene_to_file("res://scenes/fase_21.tscn")

func _on_fase2_pressed():
	if unlocked_stage >= 2:
		_play_click()
		get_tree().change_scene_to_file("res://scenes/fase_22.tscn")

func _on_fase3_pressed():
	if unlocked_stage >= 3:
		_play_click()
		get_tree().change_scene_to_file("res://scenes/fase_23.tscn")
