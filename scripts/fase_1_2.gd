...
func _soltar_item(item):
	if item.name in respostas and respostas[item.name] != "voltou":
		return 
	...
	if rect_item.intersects(rect_renov):
		area_destino = "renovavel"
	elif rect_item.intersects(rect_naorenov):
		area_destino = "nao_renovavel"
	else:
		item.position = original_pos[item.name]
		respostas.erase(item.name)
		return

	if area_destino == corretos[item.name]:
		respostas[item.name] = area_destino
		...
		_dar_feedback_erro_e_retornar(item)

func _dar_feedback_erro_e_retornar(item):
	feedback_label.visible = true
	feedback_label.text = "❌ Tente de novo! Este item está no lugar errado."
